...
function renderMap(a, b) {
    ...
    var c = createRequestObject(),
        d = "karte.php?id=" + a.getAttribute("vid") + (b ? "&l" : "");
            ...
            if (mreq = !0, c.responseText.length > 0) {
                ...
                for (var a = 0, d = _mp.mtx.length; a < d; a++)
                    for (var b = _mp.mtx[a], g = 0, k = b.length; g < k; g++) {
                        ...
                        j.setAttribute("href", "dorf3.php?id=" + h[0]);
                        if (a == 0) _("my" +
                            g).innerHTML = h[2];
...
function slm() {
    window.open("karte.php?l&id=" + _mp.mtx[3][3][0], "map", "top=100,left=25,width=1007,height=585").focus();
    return !1
}
